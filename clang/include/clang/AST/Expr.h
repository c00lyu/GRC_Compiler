...
/// \brief Represents a call to a Grc task function.
class GrcTaskCallExpr : public CallExpr {
private:
  enum { CONFIG, END_PREARG };
  bool Syn;

public:
  GrcTaskCallExpr(ASTContext &C, Expr *fn, CallExpr *Config,
                     ArrayRef<Expr*> args, QualType t, ExprValueKind VK,
                     SourceLocation RP,bool syn=true)
    : CallExpr(C, GrcTaskCallExprClass, fn, END_PREARG, args, t, VK, RP) {
    setConfig(Config);
    setSyn(syn);
  }

  GrcTaskCallExpr(ASTContext &C, EmptyShell Empty,bool syn=true)
    : CallExpr(C, GrcTaskCallExprClass, END_PREARG, Empty) {
	setSyn(syn);
  }
  ...
  void setSyn(bool syn){ Syn = syn; }
  bool isSyn()const{return Syn;}

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == GrcTaskCallExprClass;
  }
};
}  // end namespace clang
