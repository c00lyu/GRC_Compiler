...
bool CodeGenFunction::EmitSimpleStmt(const Stmt *S) {
  ...
  case Stmt::CompoundStmtClass: {
	  if(getLangOpts().GRC && (cast<CompoundStmt>(S))->isGrcParallel()){
		  BuildGrcSharedVarList();
		  CGM.getGRCRuntime().EmitPEAHead(*this);
		  ClearGrcSharedVarList();
		  EnterGrcParallel();
	  }
...
CodeGenFunction::EmitCompoundStmtWithoutScope(const CompoundStmt &S,
                                              bool GetLast,
                                              AggValueSlot AggSlot) {
	// Emit the grc parallel begin basic block
	if(S.isGrcParallel()){
		llvm::BasicBlock *GrcParallelBeginBlock = createBasicBlock("grc.parallel.begin");
		EmitBranch(GrcParallelBeginBlock);
		EmitBlock(GrcParallelBeginBlock);
	}
  ...
  if (GetLast) {
    ...
    if (hasAggregateEvaluationKind(ExprTy)) {
  	...
  	// Emit the grc parallel end basic block
	if(S.isGrcParallel()){
		llvm::BasicBlock *GrcParallelEndBlock = createBasicBlock("grc.parallel.end");
		EmitBranch(GrcParallelEndBlock);
		EmitBlock(GrcParallelEndBlock);
	}
