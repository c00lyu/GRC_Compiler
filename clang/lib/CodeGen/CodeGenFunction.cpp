...
void CodeGenFunction::addGrcTaskMetadata(llvm::Function *F){
	  llvm::Module *M = F->getParent();
	  llvm::LLVMContext &Ctx = M->getContext();

	  // Get "rpu.annotations" metadata node
	  llvm::NamedMDNode *MD = M->getOrInsertNamedMetadata("grc.tasks");
...
RValue CodeGenFunction::EmitGrcTaskCallExpr(const GrcTaskCallExpr *E,
                                            ReturnValueSlot ReturnValue){
  llvm::BasicBlock *ConfigOKBlock = createBasicBlock("tcall.begin");
  llvm::BasicBlock *ContBlock = createBasicBlock("tcall.end");

  CodeGenFunction::ConditionalEvaluation eval(*this);
  EmitBranchOnBoolExpr(E->getConfig(), ContBlock, ConfigOKBlock);
  ...
  EmitCall(E->getCallee()->getType(), Callee, E->getLocStart(),
               ReturnValue, E->arg_begin(), E->arg_end(), TargetDecl,E);
...
void CodeGenFunction::addGrcTaskCallMetadata(llvm::Instruction *Inst, llvm::Function *CalledFun,const CallExpr *CE){
