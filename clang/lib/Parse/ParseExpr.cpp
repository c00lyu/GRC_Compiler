...
/// \endverbatim
ExprResult Parser::ParseExpression(TypeCastState isTypeCast,bool IsSyn) {
  ExprResult LHS(ParseAssignmentExpression(isTypeCast,IsSyn));
...
ExprResult Parser::ParseAssignmentExpression(TypeCastState isTypeCast,bool IsSyn) {
...
#undef RTT_JOIN
  ...
  return ParsePostfixExpressionSuffix(Res,IsSyn);
}
...
/// \endverbatim
ExprResult
Parser::ParsePostfixExpressionSuffix(ExprResult LHS,bool IsSyn) {
  ...
  while (1) {
    ...
    case tok::lesslessless: {  // p-e: p-e '<<<' argument-expression-list '>>>'
      ...
      if (getLangOpts().CUDA && OpKind == tok::lesslessless) {
        ...
        if (!LHS.isInvalid()) {
          ...
          if (!LHS.isInvalid()) {
            ExprResult ECResult = Actions.ActOnGrcExecConfigExpr(getCurScope(),
                                      OpenLoc,
                                      ExecConfigExprs,
                                      CloseLoc);
            if (ECResult.isInvalid())
