...
Retry:
  ...
  if(Kind == tok::kw___gr_syn){
	  Syn = true;
	  ConsumeToken();
	  Kind  = Tok.getKind();
  }else if(Kind == tok::kw___gr_asyn){
	  Syn = false;
	  ConsumeToken();
	  Kind  = Tok.getKind();
  }
  ...
  case tok::kw___gr_for:                 // GRC: for-statement
	return ParseGRForStatement(TrailingElseLoc);
...
StmtResult Parser::ParseExprStatement(bool IsSyn) {
  // If a case keyword is missing, this is where it should be inserted.
  Token OldToken = Tok;

  // expression[opt] ';'
  ExprResult Expr(ParseExpression(NotTypeCast,IsSyn));
  if (Expr.isInvalid()) {
...
StmtResult Parser::ParseGRForStatement(SourceLocation *TrailingElseLoc) {
  assert((Tok.is(tok::kw___gr_for)) && "Not a for stmt!");
  SourceLocation ForLoc = ConsumeToken();  // eat the 'for'.

  if (Tok.isNot(tok::l_paren)) {
    Diag(Tok, diag::err_expected_lparen_after) << "__gr_for";
    SkipUntil(tok::semi);
    return StmtError();
  }
  ...
  return Actions.ActOnGRForStmt(ForLoc, T.getOpenLocation(), FirstPart.take(),
                              SecondPart, SecondVar, ThirdPart,
                              T.getCloseLocation(), Body.take());
}
