...
Sema::ActOnCallExpr(Scope *S, Expr *Fn, SourceLocation LParenLoc,
                    ...
                    Expr *ExecConfig, bool IsExecConfig, bool IsSyn) {
  // Since this might be a postfix expression, get rid of ParenListExprs.
...
ExprResult
Sema::ActOnGrcExecConfigExpr(Scope *S, SourceLocation LAtLoc,
                              MultiExprArg ExecConfig, SourceLocation RAtLoc) {
  ...
  DeclRefExpr *ConfigDR = new (Context) DeclRefExpr(
      ConfigDecl, false, ConfigQTy, VK_LValue, LAtLoc);
  MarkFunctionReferenced(LAtLoc, ConfigDecl);

  return ActOnCallExpr(S, ConfigDR, LAtLoc, ExecConfig, RAtLoc, 0,
                       /*IsExecConfig=*/true);
}
...
Sema::BuildResolvedCallExpr(Expr *Fn, NamedDecl *NDecl,
                            ...
                            ArrayRef<Expr *> Args,
                            SourceLocation RParenLoc,
                            Expr *Config, bool IsExecConfig, bool IsSyn) {
  ...
  CallExpr *TheCall;
  if (getLangOpts().CUDA && Config)
    TheCall = new (Context) CUDAKernelCallExpr(Context, Fn,
                                               cast<CallExpr>(Config), Args,
                                               Context.BoolTy, VK_RValue,
                                               RParenLoc);
  else if (getLangOpts().GRC && Config)
      TheCall = new (Context) GrcTaskCallExpr(Context, Fn,
                                                 cast<CallExpr>(Config), Args,
                                                 Context.BoolTy, VK_RValue,
                                                 RParenLoc,IsSyn);
  else
